...
# little_lm=/home1/meichaoyang/workspace/git/ASR_train/lm/diming.lm

ori_lm=/home1/meichaoyang/workspace/git/ASR_train/test/ori_1.lm
lm_2a=/home1/meichaoyang/workspace/git/ASR_train/test/2a.lm
lm_2b=/home1/meichaoyang/workspace/git/ASR_train/test/2b.lm
txt_2b=/home1/meichaoyang/workspace/git/ASR_train/test/2b.txt
...
if [ $stage -le 0 ]; then
  [ -d data/dict_grammar1 ] && rm -r data/dict_grammar1
  cp -r data/dict data/dict_grammar1
  echo "#nonterm:location" > data/dict_grammar1/nonterminals.txt

  [ -f data/lang_grammar1/G.fst ] && rm data/lang_grammar1/G.fst
  utils/prepare_lang.sh --position-dependent-phones false data/dict_grammar1 \
       "SPOKEN_NOISE" data/local/lang data/lang_grammar1
fi
...
if [ $stage -le 1 ]; then
  ...
  # everything else.
  rm -r  data/lang_grammar1{a,b} 2>/dev/null || true
...
if [ $stage -le 3 ]; then
  ...
  gzip -c ${lm_2a} > ${lang}/lm.gz
  bash utils/format_lm.sh ${lang} ${lang}/lm.gz data/dict_grammar1/lexicon.txt ${lang}

  nonterm_location=$(grep '#nonterm:location' $lang/words.txt | awk '{print $2}')

  fstrmsymbols --remove-from-output=true "echo $nonterm_location|" $lang/G.fst $lang/Ga.fst
  mv $lang/Ga.fst $lang/G.fst
...
#     gzip -c ${lm_2b} > ${lang}/lm.gz
#   bash utils/format_lm.sh ${lang} ${lang}/lm.gz data/dict_grammar1/lexicon.txt ${lang}
  python local/grammar/pre_class_G.py ${txt_2b} | tee | fstcompile --isymbols=$lang/words.txt --osymbols=$lang/words.txt | \
     fstarcsort --sort_type=ilabel > $lang/G.fst
# 2    3    #nonterm_begin <eps>
# 0    1    #nonterm_end <eps>
# 3    4    口  口    0.69314718055994
# 4    0    口  口    0.69314718055994
# 3    4    阵雨  阵雨    0.69314718055994
# 4    0    转阴  转阴    0.69314718055994
# 3    4    记得  记得    0.69314718055994
# 4    5    添衣  添衣    0.69314718055994
# 5    0    保暖  保暖    0.69314718055994
# 3    0    刮风了  刮风了    0.69314718055994
# 3    4    一定要  一定要    0.69314718055994
# 4    0    注意安全  注意安全    0.69314718055994
# 3    0    雪莉  雪莉    0.69314718055994
# 1
# EOF
...
if [ $stage -le 5 ]; then
  ...
  lang=data/lang_grammar2a
  offset=$(grep nonterm_bos $lang/phones.txt | awk '{print $2}') # 364
  clist=$(grep nonterm:location $lang/phones.txt | awk '{print $2}') # 368
  ...
  cp $tree_dir/grammar2a/words.txt $tree_dir/grammar2/words.txt
  make-grammar-fst --write-as-grammar=${write_as_grammar} --nonterm-phones-offset=$offset  $tree_dir/grammar2a/HCLG.fst \
