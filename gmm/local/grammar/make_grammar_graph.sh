...
if [ $stage -le 0 ]; then
  ...
  echo "#nonterm:contact_list" > data/dict_grammar1/nonterminals.txt
...
1    2    一遍  一遍    0.69314718055994
1    2    一  一    0.39314718055994
1    2    一下 一下 0.69314718055994
1    2    <eps>  <eps>  5.0
2    3    #nonterm_end <eps>
3
EOF
...
if [ $stage -le 5 ]; then
  ...
  make-grammar-fst --write-as-grammar=${write_as_grammar} --nonterm-phones-offset=$offset  $tree_dir/grammar2a/HCLG.fst \
                   $clist $tree_dir/grammar2b/HCLG.fst  exp/chain/tdnn_attend/grammar2/HCLG.fst
fi
